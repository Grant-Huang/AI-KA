...
- **Docker Compose**（推荐）：项目根目录 `docker compose up -d --build`，默认 **[http://localhost:8765](http://localhost:8765)**（前后端同源）。步骤摘要见 [docker-deploy.md](docker-deploy.md)。
...
- `**AIKA_REPO_ROOT`**：显式指定「视为仓库根」的路径；容器内一般为 `/app`，用于解析 `app_settings.md`、`review_skill_packages`、`.tmp` 等。
...
| `AIKA_REVIEW_SKILL_PACKAGES_ROOT`                                    | （可选）审查技能包根目录；未设置时默认为 `$AIKA_REPO_ROOT/review_skill_packages`。布局见 `docs/aika_spec/PACKAGE_LAYOUT.md`。               |
...
## 5. `app_settings.md` 与审查技能包

- **审查技能包**：关注点与组合表来自 `review_skill_packages/<package_id>/review_domain.md`（与仓库根模板 `default_skills.md` 同格式）。当前活动包 id 存于 `**app_settings.md`** 的 `**active_skill_package_id**`（默认 `package-general`）。首次若默认包下尚无 `review_domain.md`，后端会从仓库根 `**default_skills.md**` 或内置占位初始化。可挂载整个包目录，例如 `./review_skill_packages:/app/review_skill_packages`。
- **文件内容要求（摘要）**（针对 `review_domain.md`）：
  ...
  - 仓库内 `**default_skills.md**` 可作为模板；可调用 `POST /api/v1/settings/review-domain/restore-default-skills-template` 写入当前活动包，或使用 `scripts/migrate_legacy_to_skill_package.py` / `POST /api/v1/skill-packages/migrate-legacy-file`（默认源 `default_skills.md`）导入。
- `**app_settings.md**`：JSON 围栏内存放 `chunk_limit`、`chunk_strategy`、`llm_settings`、Key、`active_skill_package_id` 等；缺失或解析失败时可回退 `default_app_settings.md`。
...
- 自动编排（`agent/stream`）在检测到项目语料缺失/过期时会提示用户回到「项目初始化」重新建立索引，但**不会自动执行**转换/索引步骤。
